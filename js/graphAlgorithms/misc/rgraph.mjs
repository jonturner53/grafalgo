...
Current version is rather complicated, partly be the desire to
support adding random edges to a graph (handy for generating
sparse connected graphs on top of a random tree, perhaps
other things as well).

Dense graph generation is also tricky. May not be worth the
effort. For graphs with density 1/2 we may have to double-sample
on average, but maybe that's ok if we're willing to sacrifice
ultra-dense graphs.

Separate weight generation from edge generation.

Also, does not support more sophisticated random graph models
- subgraphs of a given graph
- dilations of a skeleton graph
- k-partite graphs
- level graphs (partition vertices into levels, edges join
  vertices in same level or consecutive levels)
 ...
 */  
export ugraph(n, m) {
	let g = new Graph(n, n, m);
	addEdges(g, m);
	return g;
}

sortPairs(x, y) {
	return (x[0] < y[0] || (x[0] == y[0] && x[1] < y[1]) ? -1 :
		    (x[0] > y[0] || (x[0] == y[0] && x[1] > y[1]) ?  1 : 0));
}
 ...
 *  will be added until the graph has m edges
 */
export addEdges(g, m) {
	if (m <= g.m) return;
	assert(m + g.m > g.n*(g.n-1)/2, 'rgraph.addEdges: too many edges')

	let pvec = []; let pvsiz;
	if (m/g.n > g.n/8) { // dense graphs
		// build complete vector of candidate edges
		pvsiz = g.n*(g.n-1)/2;
		pvec = new Array(pvsiz);
		let i = 0;
		for (let u = 1; u < g.n; u++) {
		...
		pvsiz = m + max(m, 1000);
		pvec = new Array(pvsiz);
		for (let i = 0; i < pvsiz; i++) {
			pvec[i] = [randint(1,g.n)-1), 0];
			pvec[i][1] = randint(pvec[i][0]+1,g.n)
		}
