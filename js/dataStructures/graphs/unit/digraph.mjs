...
try {
	...
	let n = 10; let ecap = 30;
	let g = new Digraph(n, ecap);

	matches(g.n, n, 'a1');
	matches(g.m, 0, 'a2');
	matches(g, '{a[] b[] c[] d[] e[] f[] g[] h[] j}', 'a3');
	g.join(1, 4); g.join(1, 3); g.join(2, 4); g.join(3, 5);
	g.join(5, 2); g.join(7, 1);
	matches(g, '{a[d c] b[d] c[e] d[] e[b] f[] g[a] h[] j}', 'a4');
	g.join(5, 1); g.join(2, 7); g.join(3, 7); g.join(8, 5);
	matches(g, '{a[d c] b[d g] c[e g] d[] e[b a] f[] g[a] h[e] j}', 'a5');
	let elist = [];
	elist.push(g.join(5, 9));
	elist.push(g.join(7, 8));
	elist.push(g.join(9, 4)); 
	matches(g, '{a[c d] b[d g] c[e g] d[] e[a b i] f[] g[a h] h[e] i[d] j}',
			'a6');
	g.delete(g.findEdge(1, 3));
	g.delete(g.findEdge(5, 2));
	matches(g, '{a[d] b[d g] c[e g] d[] e[a i] f[] g[a h] h[e] i[d] j}', 'a7');
	...
	let g2 = new Digraph(g.n, g.m);
	for (let e = g.first(); e; e = g.next(e)) g2.join(g.tail(e),g.head(e));
	matches(g.toString(), g2.toString(), 'b1');
	matches(g, g2, 'b2');
	matches(g2, '{a[d] b[d g] c[e g] d e[a i] f g[a h] h[e] i[d] j}' ,'b3');
	g2.clear();
	matches(g2, '{j}', 'b4'); 
	matches(g2.m, 0, 'b5'); 

	g = Digraph.fromString('{a[d b] b[c] c[d] d[b a] e[d b a]}');
	matches(!!g, true, 'c0');
	...
	g = Digraph.fromString('{a[d:1 b:2] b[c:3] c[d:4] ' +
							'd[b:5 a:6] e[d:7 b:8 a:9]}');
