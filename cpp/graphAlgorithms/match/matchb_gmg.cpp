 ...
 *  result is returned
 */
matchb_gmg::matchb_gmg(Graph_w& g1, List_g<edge>& match) : g(&g1) {
	state = new stype[g->n()+1];
	mEdge = new edge[g->n()+1];
	pEdge = new edge[g->n()+1];
	z = new double[g->n()+1];

	h1o = new Heap_dd<double>(g->n(),2);
	h1e = new Heap_dd<double>(g->n(),2);
	h2 = new Heap_dd<double>(g->M(),2);
	h3 = new Heap_dd<double>(g->M(),2);
 ...
 */
double matchb_gmg::augment(edge e) {
	vertex u = g->left(e);
	double pathWeight = 0;;
	mEdge[u] = 0;
	while (pEdge[u] != 0) {
	...
	if (u == v) Util::fatal("matchb_gmg::augment: graph not bipartite");
