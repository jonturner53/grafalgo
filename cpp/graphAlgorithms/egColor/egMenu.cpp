...
egMenu::egMenu(GroupGraph& g, int edgeColors[]) : egColor(g, edgeColors) {
	...
	for (vertex u = 1; u <= g.n(); u++)
		menus[u] = new ClistSet(colorBound);
	fc = new int[g.M()+1];
	...
	gx = new int[gp->M()+1];
	dymatch = new dynamicMatch*[gp->n()+1];
	for (vertex v = gp->firstOut(); v != 0; v = gp->nextOut(v)) {
		...
		for (edge e = g.firstAt(v); e != 0; e = gp->nextAt(v,e))
			gx[e] = i++;
		dymatch[v] = new dynamicMatch(*mgraf[v], dv);
	}
}
...
bool egMenu::inMenu(int grp, int c) const {
 ...
 */
int egMenu::menuSize(int grp) const {
	int cnt = 0;
 ...
 *  @param grp is a group number
 */
void egMenu::addColor(int c, int grp) {
 ...
 *  @param grp is a group number
 */
void egMenu::removeColor(int c, int grp) {
	edge e = gp->firstEdgeInGroup(grp);
	if (e == 0) return;
	vertex u = gp->input(e);
 ...
 */
int egMenu::value(int c, int grp) {
	int val = 0;
	for (edge e = gp->firstEdgeInGroup(grp); e != 0;
		  e = gp->nextEdgeInGroup(grp,e)) {
		vertex v = gp->output(e);
 ...
 */
int egMenu::growMenu(int grp, int c) {
 ...
 *  @return the deficit of grp after c has been added
 */
int egMenu::shrinkMenu(int grp, int c) {
	removeColor(c,grp);
	vertex u = gp->input(gp->firstEdgeInGroup(grp));
	free(c, u);
 ...
 */
bool egMenu::isConsistent(vertex v) const {
	int dv = gp->degree(v);
	dynamicMatch& dm = *dymatch[v];
