 ...
 */
void beRelaxedSplit(Wdigraph& g, int color[]) {
	int bmax = 0;
	...
	buildFlograf(g,k,hi,fg); maxFloMin(fg,"dinic");
	...
	Dlist avail[g.n()+1]; // list of available colors
	for (vertex u = 1; u <= g.n(); u++) {
		d[u] = g.degree(u);
		avail[u].resize(bmax+g.maxDegree());
		for (int c = 1; c <= k-1; c++) avail[u].addLast(c);
	}
	Dheap<int> edges(g.M(),2); // edges ordered by max endpoint degree
	for (edge e = g.first(); e != 0; e = g.next(e)) {
