...
/** Constructor for Graph_f objects.
 ...
 */
Graph_f::Graph_f(int numv, int maxe, int s1, int t1) 
	: Graph_d(numv, maxe), s(s1), t(t1) {
	makeSpace(numv,maxe);
}

Graph_f::~Graph_f() { freeSpace(); }

/** Allocate and initialize dynamic storage for Graph_f.
 *  @param numv is the number of vertices to allocate space for
 *  @param maxe is the number of edges to allocate space for
 */
void Graph_f::makeSpace(int numv, int maxe) {
	floInfo = new FloInfo[maxe+1];
}

/** Free space used by graph. */
void Graph_f::freeSpace() { delete [] floInfo; }

/** Resize a Graph_f object.
 *  The old value is discarded.
 *  @param numv is the number of vertices to allocate space for
 *  @param maxe is the number of edges to allocate space for
 */
void Graph_f::resize(int numv, int maxe) {
	freeSpace(); Graph_d::resize(numv,maxe); makeSpace(numv,maxe);
}

/** Expand the space available for this Graph_f.
 *  Rebuilds old value in new space.
 *  @param size is the size of the resized object.
 */
void Graph_f::expand(int numv, int maxe) {
	if (numv <= n() && maxe <= M()) return;
	Graph_f old(this->n(),this->M()); old.copyFrom(*this);
	resize(numv,maxe); this->copyFrom(old);
}

/** Copy into list from source. */
void Graph_f::copyFrom(const Graph_f& source) {
 ...
 */
bool Graph_f::readAdjList(istream& in) {
 ...
 */
edge Graph_f::join(vertex u, vertex v) {
	edge e = Graph_d::join(u,v); setFlow(e,0);
	return e;
}
 ...
 *  @return number of new edge from u to v
 */
edge Graph_f::joinWith(vertex u, vertex v, edge e) {
	edge ee = Graph_d::joinWith(u,v,e); setFlow(ee,0);
	return ee;
}

/** Remove flow from all edges.
 */
void Graph_f::clearFlow() {
 ...
 */
void Graph_f::addFlow(vertex v, edge e, flow ff) {
	if (tail(e) == v) {
 ...
 *  @return the string
 */
string Graph_f::edge2string(edge e) const {
	string s;
 ...
 *  @return the string
 */
string Graph_f::adjList2string(vertex u) const {
	string s = "";
 ...
 *  @return the string
 */
string Graph_f::toDotString() const {
	string s = "digraph G {\n";
