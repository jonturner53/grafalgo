 ...
 */
Djsets_flt::Djsets_flt(int n) : Adt(n) { makeSpace(); clear(); }

/** Destructor for Djsets_flt. */
Djsets_flt::~Djsets_flt() { freeSpace(); }

/** Allocate and initialize space for list.  */
void Djsets_flt::makeSpace() { node = new pnode[n()+1]; }

/** Free dynamic storage used by list. */
void Djsets_flt::freeSpace() { delete [] node; }

/** Resize a Djsets_flt object.
 *  The old value is discarded.
 *  @param n is the size of the resized object.
 */
void Djsets_flt::resize(int n) { freeSpace(); Adt::resize(n); makeSpace(); }

/** Expand the space available for this Djsets_flt.
 *  Rebuilds old value in new space.
 *  @param n is the size of the resized object.
 */
void Djsets_flt::expand(int n) {
	if (n <= this->n()) return;
	Djsets_flt old(this->n());
	old.copyFrom(*this);
	resize(n);
	this->copyFrom(old);
}
...
void Djsets_flt::clear() {
	for (index x = 0; x <= n(); x++) { p(x) = x; rank(x) = 0; }
}

/** Copy another Djsets_flt object to this one.
 *  @param source is another Djsets_flt object
 */
void Djsets_flt::copyFrom(const Djsets_flt& source) {
 ...
 *  the given sets
 */
index Djsets_flt::link(index x, index y) {
 ...
 *  @return the canonical element of the set containing x
 */
int Djsets_flt::findroot(int x) const {
	assert(valid(x));
	if (x == p(x)) return(x);
 ...
 */
string Djsets_flt::toString() const {
	string s = "{";
