...
class Set_h : public Adt {
public:
		Set_h(int=10, bool=true);
		Set_h(const Set_h&);
		Set_h(Set_h&&);
		~Set_h();

	// operators
	Set_h& operator=(const Set_h&);
	Set_h& operator=(Set_h&&);
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
Set_h<E,H>::Set_h(const Set_h& src) : Adt(src.n()) {
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
Set_h<E,H>::Set_h(Set_h&& src) : Adt(src.n()) {
	autoExpand = src.autoExpand;
	nb = src.nb;
	bktMsk = src.bktMsk; fpMsk = src.fpMsk; indexMsk = src.indexMsk;
...
/** Destructor for Set_h. */
template<class E, uint32_t (*H)(const E&, int)>
Set_h<E,H>::~Set_h() { freeSpace(); }

/** Allocate and initialize space for list.
 *  @param size is number of index values to provide space for
 */
template<class E, uint32_t (*H)(const E&, int)>
void Set_h<E,H>::makeSpace() {
	nb = numBuckets();
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
Set_h<E,H>& Set_h<E,H>::operator=(const Set_h& src) {
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
Set_h<E,H>& Set_h<E,H>::operator=(Set_h&& src) {
	if (this == &src) return *this;
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline int Set_h<E,H>::first() const { return idx->firstIn(); }
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline int Set_h<E,H>::next(int id) const { return idx->nextIn(id); }
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline bool Set_h<E,H>::empty() const { return size() == 0; }
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline bool Set_h<E,H>::contains(const E& elem) const { return find(elem)!=0; }
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline bool Set_h<E,H>::valid(int x) const { return idx->isIn(x); }

/** The size of the mapping.
 *  @return the number of mapped index values.
 */
template<class E, uint32_t (*H)(const E&, int)>
inline int Set_h<E,H>::size() const { return idx->getNumIn(); }
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline const E& Set_h<E,H>::retrieve(index x) const {
	assert(valid(x));
	return eVec[x]; 
}
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
int Set_h<E,H>::find(const E& elem) const {
	index x;
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
inline index Set_h<E,H>::insert(const E& elem) {
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
index Set_h<E,H>::insert(const E& elem, index x) {
 ...
 */
template<class E, uint32_t (*H)(const E&, int)>
void Set_h<E,H>::remove(const E& elem) {
	index x;
	// check bucket in the first half of the bucket array
	uint32_t h = H(elem,0);
...
template<class E, uint32_t (*H)(const E&, int)>
void Set_h<E,H>::clear() {
	while (first() != 0) remove(retrieve(first()));
}

/** Create a string representation of the Set_h.
 *  @return the string
 */
template<class E, uint32_t (*H)(const E&, int)>
string Set_h<E,H>::toString() const {
	stringstream ss; ss << "{";
