 ...
 *  instead.
 */
template<class T> class NonblockingQ {
public:		NonblockingQ(int=4);
		~NonblockingQ();
...
template<class T>
inline NonblockingQ<T>::~NonblockingQ() { delete [] buf; }
 ...
 */
template<class T>
inline void NonblockingQ<T>::reset() {
	rc.store(0); wc.store(0); wcs.store(0);
}
 ...
 */
template<class T>
inline bool NonblockingQ<T>::empty() const { return rc.load() == wc.load(); }

/** Determine if queue is full.
 *  @return true if the queue is full, else false
 */
template<class T>
inline bool NonblockingQ<T>::full() const {
...
template<class T>
inline string NonblockingQ<T>::toString() const {
	stringstream ss;
