 ...
 */
template<class K>
Dheap<K>::Dheap(int n, int dd) : Adt(n), d(dd) { makeSpace(); init(); }

/** Destructor for Dheap class. */
template<class K>
Dheap<K>::~Dheap() { freeSpace(); }

/** Allocate space for Dheap.
 *  @param size is number of index values to provide space for
 */
template<class K>
void Dheap<K>::makeSpace() {
	h = new index[n()+1]; pos = new int[n()+1]; kee = new K[n()+1];
}

/** Initialize a Dheap object. */
template<class K>
void Dheap<K>::init() {
 ...
 */
template<class K>
inline K Dheap<K>::key(index i) const { assert(valid(i)); return kee[i]; }
 ...
 */
template<class K>
inline bool Dheap<K>::member(index i) const {
